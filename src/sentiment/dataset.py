"""Слой данных проекта: схема, маппинг меток, загрузка и валидация CSV.

Здесь живёт всё, что нужно, чтобы получить из файла ``data/sentiment_dataset.csv``
пригодный для работы DataFrame. Модуль не зависит от EDA и будет переиспользован
этапами предобработки, обучения и инференса.
"""
...
class DatasetSpec:
    ...
    переиспользовать (обучение, инференс) без обращения к модулю EDA.
    """
...
class ReviewsDatasetLoader:
    ...
    графиками занимаются другие компоненты (SRP).
    """
    ...
    def _coerce_dtypes(self, frame: pd.DataFrame) -> pd.DataFrame:
        """Привести колонки к ожидаемым типам.

        Метка сохраняется как расширяемый целой тип ``Int64``, чтобы пропуск
        можно было посчитать и отбросить осознанно, а не молча превратить в ноль.
        """
...
class DatasetValidator:
    """Проверяет содержательную целостность датасета и чистит его.
